...
class GatedShift(nn.Module):
    def __init__(self, net, n_segment, n_div, is_gsf=False):
...
def make_temporal_shift(net, clip_len, is_gsm=False, is_gsf=False, step=1):

    def _build_shift(net):
        if is_gsm or is_gsf:
            return GatedShift(net, n_segment=clip_len, n_div=4, is_gsf=is_gsf)
        else:
